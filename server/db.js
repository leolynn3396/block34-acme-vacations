...
const createPlace = async(name)=> {
    const SQL = `
    INSERT INTO places(id, name) VALUE($1, $2) RETURNING *
    `;
    const response = await client.query(SQL, [uuid.v4(),name]);
    return response.rows[0];
};
const createVacation = async({place_id, user_id, departure_date})=> {
    const SQL = `
    INSERT INTO vacations(id, place_id, user_id, departure_date)) VALUE($1, $2, $3, $4) RETURNING *
    `;
